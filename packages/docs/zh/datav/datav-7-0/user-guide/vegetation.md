...
-   **通用**：设置植被的通用配置参数项。
    ...
    **阴影**
    ...
    -   **发射阴影**：通过打开或关闭**发射阴影**的开关，控制植被的阴影效果样式。
...
-   **模型**：设置植被的模型样式。
    ...
    **模型映射**
    ...
    单击**模型映射**右侧的![加号图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0201929951/p89097.jpg)或![垃圾桶图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3208049161/p89098.jpg)图标，添加或删除一个模型映射。 单击![横排列图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0201929951/p95437.jpg)或![竖排列图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0201929951/p94077.jpg)图标配置多个模型映射的排列方式。单击![复制图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5896941261/p271562.jpg)图标，即可复制当前选中的模型映射进行配置并新增一个同样配置的模型映射。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**植被**图层，在画布中可以看到如下图所示的植被的蓝图编辑器配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8194039861/p693754.png)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如植被配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
