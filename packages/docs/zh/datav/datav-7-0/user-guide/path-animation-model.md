...
-   **动效**
    ...
    **模型动画**
    ...
    -   **循环模式**：设置路径动画模型播放的循环模式，可选**重复播放**、**单次播放**和**反向播放**。
    ...
    **扫光动画**
    ...
    -   **扫描轴向**：当锋面类型选择**平面**时，扫描轴向包括X、Y和Z轴。
        
    -   **正负方向**：设置扫光动画的正负方向，包括正方向和负方向。
        
    -   **颜色**：设置扫光动画的颜色，请参见[颜色选择器说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)修改颜色。
    ...
    **路径动画**
    ...
    -   **循环模式**：设置路径动画模型播放的循环模式，可选**重复播放**和**单次播放**。
...
-   **交互**：设置路径动画模型的交互样式。
    ...
    **点击区域**
    ...
    设置鼠标单击路径动画模型加载模型的区域选项，包括**局部**和**整体**两个选择。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**路径动画模型**图层，在画布中可以看到如下图所示的路径动画模型的蓝图编辑器配置参数。![蓝图配置面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5813754561/p447174.png)
    ...
    -   **动作**
        ...
        **请求运动路径**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如路径动画模型配置了API数据源为`https://api.test`，传到**请求运动路径**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
