...
-   呼吸气泡配置面板参数说明。
    ...
    -   **通用**
        ...
        **混合模式**
        ...
        设置呼吸气泡层的混合叠加渲染模式，可选**添加模式**和**正常模式**。
    ...
    -   **图形**
        ...
        **颜色**
        ...
        设置呼吸气泡的颜色，请参见[颜色选择器说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)修改颜色。
        
        **序列帧合成图片**
        
        在**序列帧图片**区域，选择序列帧合成图片的类型。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**呼吸气泡**图层，在画布中可以看到如下图所示的呼吸气泡的蓝图编辑器配置参数。![呼吸气泡蓝图配置参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2909548461/p422409.png)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如呼吸气泡配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
