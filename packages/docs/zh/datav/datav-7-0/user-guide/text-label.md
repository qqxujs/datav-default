...
-   **全局**
    ...
    **自动布局**
    ...
    打开开关，自动根据标签内容的长度来定义每个表格的宽度；关闭开关，内容无序排布，需要自定义设置行列数及行列间距，按照图表宽度等比排列。
    
    **说明**
    
    只有在关闭**自动布局**配置项时，**行数**、**列数**、**标签列间距**和**标签行间距**配置项才会显示。
    ...
    **标签列间距**
    ...
    标签之间每一列之间的横向间距，单位为px。
    
    **标签行间距**
    
    标签之间每一行之间的纵向间距，单位为px。
...
-   **默认标签配置**：标签的默认样式，即数据中没有type字段或type字段与设置的标签类别名不一致时，所对应的标签的样式。
    ...
    **内容边距**
    ...
    内容与标签块的内边距，对所有标签生效，单位为px。
    
    **圆角半径**
    
    标签块四个角的弧度 ，对所有标签生效，单位为px。
    
    **背景颜色**
    
    默认标签块（数据中没有定义type值）的背景颜色，请参见[颜色选择器说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)进行修改。
...
（可选）设置标签类别名。若`type`值与**样式>自定义标签系列>标签类别名**配置项匹配，组件使用**自定义标签系列**中定义的样式进行渲染。若不匹配或为空，则使用**样式>默认标签配置**进行渲染。
    ...
    -   **动作**
        ...
        **导入数据接口**
        ...
        [
          {
            "content": "时尚达人",
            "type": "series1"  //与样式>自定义标签系列>标签类别名对应
          }
        ]
        ```
