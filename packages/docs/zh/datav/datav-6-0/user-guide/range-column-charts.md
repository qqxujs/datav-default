  区间柱状图组件配置项与数据交互详解-DataV数据可视化-阿里云
...
-   **图形**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1255536761/p561430.png)
    ...
    **柱子标注**
    ...
    -   **标注字段**：标注的内容需要和数据面板中字段的内容对应，默认获取range字段内容。
...
选中**启用**，开启组件交互功能。**区间柱状图**组件拥有交互配置，当单击选中某个数据项时抛出回调值，默认抛出数据中的x、y1、y2和colorField字段。具体配置请参见[如何配置数字翻牌器组件的回调ID](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards)。

**当图例被点击时**

选中**启用**，开启组件交互功能。**区间柱状图**组件拥有交互配置，当单击某个图例时抛出回调值，默认抛出数据中的value和checked字段。具体配置请参见[如何配置数字翻牌器组件的回调ID](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards)。
