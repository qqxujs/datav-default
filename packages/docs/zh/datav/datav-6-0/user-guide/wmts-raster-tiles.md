...
| 参数   | 说明                        |
| ---- | ---- |
| 渲染顺序 | 设置WMTS栅格瓦片的渲染顺序，值越大瓦片越靠前。 |
  * 图形

| 参数    | 说明                              |
| ---- | ---- |
| 透明度   | 拖动滑块设置WMTS栅格瓦片的透明度系数，取值范围为0\~1。 |
| HSB调整 | 设置WMTS栅格瓦片的HSB值，包括色调、饱和度和亮度。    |
...
| ---- | ---- |
| url                   | WMTS栅格瓦片的服务地址。 |
| projType              | WMTS栅格瓦片的投影类型。 |
| activeLayer           | WMTS栅格瓦片的图层名称。 |
| headers               | WMTS栅格瓦片的请求头。  |
| customParameters      | WMTS栅格瓦片的服务参数。 |
| customLayerParameters | WMTS栅格瓦片的图层参数。 |
| serviceMode           | WMTS栅格瓦片的服务模式。 |
...
| 动作     | 说明                                                                                                                           |
| ---- | ---- |
| 请求数据接口 | 重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如WMTS栅格瓦片配置了API数据源为http://api.test，传到请求数据接口动作的数据为{ id: '1'}，则最终请求接口为http://api.test?id=1。 |
