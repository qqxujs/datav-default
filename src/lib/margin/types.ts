...
export interface TypeConfigMargin extends TypeConfigBase {
  name: string //	显示名	string	是	无。
  type: TypeControl.margin //	类型	string	是	无。
  default?: any //	默认值	string	否	不填时值为空。
  components?: TypeConfigMarginComponents[] //	配置项模块	array	否	默认值为["top","bottom","left","right"]。支持的模块包括"top"、"bottom"、"left"和"right"。
