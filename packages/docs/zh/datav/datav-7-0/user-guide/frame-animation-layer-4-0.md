...
-   **图形**：设置帧动画层的图形配置项。
    ...
    **动图形状**
    ...
    选择动图形状的样式，您可以单击样式右侧的下拉选项框，选择系统内置五种样式；也可在下拉框面板的输入框内自定义输入帧动画图片的URL地址，将远程服务器上的帧动画图片作为动图；或者单击输入框右侧的![上传按键](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3366732361/p310531.jpg)图标，上传本地帧动画图片作为动图。
    
    **动图尺寸**
    
    设置动图的尺寸值，取值范围为0~400，单位为px。
...
-   **标签**：设置帧动画层内标签的样式参数。
    ...
    **显示方式**
    ...
    设置帧动画层的标签显示方式，包括**点击**和**悬浮**两种方式。
    
    **相对位置**
    
    设置帧动画层的标签的相对位置样式，包括**左侧**、**中心**、**右侧**、**顶部**和**底部**。
...
-   **交互**：帧动画层的交互配置项。
    ...
    **点击态**
    ...
    设置图形、动效和标签，可通过![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，确认在**图形交互**点击态是否启用。
    
    -   **图形**：设置**点击态**时帧动画层图形样式，包括**动图形状**、**动图尺寸**、**图标旋转**、**相对位置**、**相对偏移**。
        
    -   **动效**：设置**点击态**时帧动画层动效，包括**播放速度**。
        
    -   **标签**：设置**点击态**时帧动画层标签，包括**显示标签**、**相对位置**、**相对偏移**、偏移方式、偏移量、水平字段、垂直字段、**内容样式**、。
        
    
    **悬浮态**
    
    设置图形、动效和标签，可通过![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，确认在**图形交互**悬浮态是否启用。
    
    -   **图形**：设置**悬浮态**时帧动画层图形样式，包括****动图形状**、**动图尺寸**、**图标旋转**、**相对位置**、**相对偏移**。**
        
    -   **动效**：设置**悬停态**时帧动画层动效，包括**播放速度**。
        
    -   **标签**：设置**悬浮态**时帧动画层标签，包括**显示标签**、**相对位置**、**相对偏移**、**内容样式**、**背景样式**。
    ...
    **条件样式**：单击右侧的![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528933.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528934.png)图标，添加或删除一个条件样式。 单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528936.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528937.png)图标配置多个条件样式的排列样式。单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528938.png)图标，即可复制当前选中条件样式配置内容并新增一个同样配置的条件样式。具体样式配置说明，请参见[条件树使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/conditional-tree-instructions)。
    ...
    -   **图形**：打开开关，配置该样式中的图形配置项；关闭开关，无法配置该样式下的图形配置项。
        ...
        **动图形状**
        ...
        选择该条件下的动图形状样式，您可以单击样式右侧的下拉选项框，选择系统内置五种样式；也可在下拉框面板的输入框内自定义输入帧动画图片的URL地址，将远程服务器上的帧动画图片作为动图；或者单击输入框右侧的![上传按键](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3366732361/p310531.jpg)图标，上传本地帧动画图片作为动图。
        
        **动图尺寸**
        
        设置该条件下的动图的尺寸值，取值范围为0~400，单位为px。
...
3.  查看蓝图配置参数。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9300188371/p882103.png)
    ...
    -   **动作**
        ...
        **请求数据**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如帧动画层配置了API数据源为`https://api.test`，传到**请求数据**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
