...
-   **图形**
    ...
    **柱图颜色**
    ...
    柱状图的颜色样式，具体请参见[颜色映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/instructions-of-color-mapper)。
...
3.  查看蓝图配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3661460071/p740516.png)
    ...
    -   **动作**
        ...
        **请求3D柱状图接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如3D柱状图配置了API数据源为`https://api.test`，传到**请求3D柱状图接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
