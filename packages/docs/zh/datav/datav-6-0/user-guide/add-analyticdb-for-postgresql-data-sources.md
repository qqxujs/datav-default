  在DataV中添加AnalyticDB for PostgreSQL数据源-DataV数据可视化-阿里云
...
5.  填写数据库信息。
    ...
    **VPC ID**
    ...
    VPC实例的ID，此VPC为您的AnalyticDB for PostgreSQL实例所在的VPC。可进入[AnalyticDB for PostgreSQL控制台](https://gpdbnext.console.aliyun.com/gpdb/)中获取。
    
    **实例ID**
    
    VPC下数据库实例的ID，可在[AnalyticDB for PostgreSQL控制台](https://gpdbnext.console.aliyun.com/gpdb/)中获取。
    ...
    **重要** 此处的URL地址不是官网页面的URL，也不是本机的IP，是需要DataV服务器能够通过公网或阿里云部分Region内网访问您数据库的URL地址。
