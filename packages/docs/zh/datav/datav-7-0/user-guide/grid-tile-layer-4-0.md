...
-   **通用**：栅格瓦片层通用配置参数。
    ...
    **可见级别**
    ...
    设置栅格瓦片层的可见级别值，取值范围为0~22，单位级，在设定的级别范围内栅格瓦片层可见，级别之外不可见。
    
    **不透明度**
    
    设置栅格瓦片层的不透明度值，取值范围为0~100，单位%。数值越大地图样式越不透明。
    ...
    -   **瓦片分类**：选择地图层的瓦片类型，包括**常规瓦片**和**WMS瓦片**。
        ...
        -   **常规瓦片**：常规瓦片类型的配置项参数。仅在**瓦片分类**选择**常规瓦片**可配置。
            ...
            **说明**
            ...
            您可以删除下拉框中的预设地图，并填写自定义的瓦片地址设置自定义瓦片地址。
            ...
            **缩放偏移**
            ...
            拖动滑块调整瓦片层缩放偏移的值，取值范围为-10~10。仅在父容器使用自定义坐标系时才会生效，默认值为0。
            
            **TMS协议**
            
            打开开关，栅格瓦片层内容遵循TMS协议；关闭开关，栅格瓦片层内容不遵循TMS协议。
        ...
        -   **WMS瓦片**：常规瓦片类型的配置项参数。仅在**瓦片分类**选择**WMS瓦片**时可配置。
            ...
            **图像样式**
            ...
            单击下拉框选择一个图像样式，可选**image/png**或**image/jpeg**。当含有透明区域服务时，选择**image/png**。
