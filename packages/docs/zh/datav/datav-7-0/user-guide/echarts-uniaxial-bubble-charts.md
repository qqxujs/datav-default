...
## **图表样式**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6197647761/p573597.png)

## 样式面板![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0968862961/p709280.png)
...
-   **渲染引擎**：设置气泡图的渲染引擎。包括**canvas**和**svg**。
...
-   **提示框**：单击右侧的**眼睛**图标显示提示框。
    ...
    -   **坐标轴指示器**：单击右侧的**眼睛**图标显示坐标轴指示器。
        ...
        -   **位置**：提示框相对于坐标轴的位置。包括**top**和**bottom**。
...
-   **单轴**
    ...
    -   **左边距**：单击**+**或**\-**号，或手动输入数值，调整单轴左边距，单位px。
        
    -   **上边距**：单击**+**或**\-**号，或手动输入数值，调整单轴上边距，单位px。
        
    -   **右边距**：单击**+**或**\-**号，或手动输入数值，调整单轴右边距，单位px。
        
    -   **下边距**：单击**+**或**\-**号，或手动输入数值，调整单轴下边距，单位px。
    ...
    -   **名称位置**：单轴名称的位置，可选，包括**起点**、**终点**或**居中**，默认值为 **终点**。
        
    -   **名称样式**
        
        -   **颜色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改单轴名称文本的颜色。
    ...
    -   **轴线**：单击右侧的**眼睛**图标显示单轴轴线。
        
        -   **线样式**
            
            -   **颜色**：单击下拉框，选择轴线颜色的填充样式，包括**颜色填充**和**渐变填充**。参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改单轴轴线的颜色。
                
            -   **宽度**：单击**+**或**\-**号，或手动输入数值，修改单轴轴线的宽度。
                
            -   **类型**：坐标轴轴线的类型。可选，包括**实线**、**虚线**和**点线**。
                
            -   **透明度**：单击**+**或**\-**号，或手动输入数值，修改单轴轴线的透明度。范围为 \[0,1\]。
                
    -   **刻度线**：单击右侧的**眼睛**图标显示单轴刻度。
        
        -   **和标签对齐**：在**留白**为 true 时有效，保证刻度线和标签对齐。
    ...
    -   **刻度标签**：单击右侧的**眼睛**图标显示径向轴刻度标签。
        
        -   **分割间隔**：设置标签和标签之间的显示间隔，例如：0表示显示所有标签，1表示隔一个标签显示。
    ...
    -   **分割线**：单击右侧的**眼睛**图标显示分割线。
        ...
        -   **线样式**
            ...
            -   **颜色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改单轴分割线的颜色。
                
            -   **宽度**：单击**+**或**\-**号，或手动输入数值，修改单轴分割线的线宽。
                
            -   **类型**：单轴分割线的类型。可选，包括**实线**、**虚线**和**点线**。
                
            -   **透明度**：单击**+**或**\-**号，或手动输入数值，修改单轴分割线的透明度，范围为 \[0,1\]。
                
    -   **提示框**
        
        -   文本样式
            
            -   **颜色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改提示框的颜色。
...
-   **系列**：单击右侧的![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528933.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528934.png)图标，添加或删除一个条件样式。 单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528936.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528937.png)图标配置多个条件样式的排列样式。单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528938.png)图标，即可复制当前选中的条件样式配置并新增一个同样配置的条件样式。
    ...
    -   **标记图形**：自定义标记图形，可选，默认值为**圆**。标记类型包括 **无**，**圆**，**矩形**，**圆角矩形**，**三角形**，**菱形**， **图钉**和**箭头** ，详细请参考[ECharts官网](https://echarts.apache.org/zh/option.html#series-line.markPoint)。
        
    -   **气泡大小映射系数**：设置气泡大小的映射比例。
    ...
    -   **标签**：单击右侧的**眼睛**图标显示标签样式。
        ...
        -   **位置**：设置标签相当于气泡图的位置，包括但不限于**top**、**left**和**inside**。
            
        -   **定位距离**：设置标签相当于气泡图上边框线的距离。
...
## 数据源面板![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0968862961/p709295.png)
...
## 高级面板![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0968862961/p709296.png)
...
打开开关，开启组件交互功能。当单击选中某个数据项时抛出回调值，默认抛出数据中的`x`、`value`和`s`字段。具体配置请参见[组件交互配置](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。
...
3.  查看蓝图配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0968862961/p709308.png)
    ...
    -   **事件**
        ...
        **当点击数据项时**
        ...
        当单击单轴气泡图的数据时抛出的事件，同时抛出对应的数据项。
        
        **当图例点击时**
        
        当单击单轴气泡图的图例时抛出的事件，同时抛出对应的数据项。
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如单轴气泡图配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
