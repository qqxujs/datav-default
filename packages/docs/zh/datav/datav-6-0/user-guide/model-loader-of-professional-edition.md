...
![模型加载器数据面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7663587561/p440169.png)
...
如果您在创建三维城市构建器项目时，使用了空间数据项目模式，并且项目内包含了模型加载器的数据内容。在场景编辑器内配置模型加载器的数据面板时，您还可以使用空间构建数据源获取空间地图上模型加载器的数据， 具体步骤如下：
...
1.  在当前场景编辑器页面，单击左侧图层列表中模型加载器右侧的![导出到蓝图编辑器](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89089.jpg)图标。
2.  单击页面左上角的![蓝图图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89087.jpg)图标。
3.  在蓝图编辑器配置页面，单击导入节点栏内的模型加载器图层，在画布中可以看到如下图所示的模型加载器的蓝图编辑器配置参数。![蓝图配置面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/4195587561/p446852.png)
    -   事件
    ...
    -   动作
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如模型加载器配置了API数据源为`http://api.test`，传到请求数据接口动作的数据为`{ id: '1'}`，则最终请求接口为`htp://api.test?id=1`。
