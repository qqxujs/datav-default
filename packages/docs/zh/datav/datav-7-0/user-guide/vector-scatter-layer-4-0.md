...
-   **图形**：设置矢量散点层的图形配置项。
    ...
    **矢量形状**
    ...
    设置矢量散点的形状，请参见[形状映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/shape-mapper-instructions)，配置不同矢量散点的映射形状样式。
    
    **散点半径**
    
    设置散点的形状半径大小值，请参见[尺寸映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/dimension-mapper-instructions)，配置不同散点的映射半径尺寸样式。
    ...
    **散点颜色**
    ...
    设置散点的颜色，请参见[颜色映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/instructions-of-color-mapper)，配置不同散点的映射颜色样式。
...
-   **标签**：设置矢量散点层内标签的样式参数。
    ...
    **显示方式**
    ...
    设置矢量散点层的标签显示的方式，包括**全显**、**点击**和**悬浮**三种。
    
    **相对位置**
    
    设置矢量散点层的标签的相对位置样式，包括**左侧**、**中心**、**右侧**、**顶部**和**底部**。
    
    **相对偏移**
    
    设置矢量散点层的标签在水平和垂直方向的相对偏移值，单位为px，取值范围为-100~100。
    
    **内容样式**
    
    设置矢量散点层标签的内容样式。
    
    -   **每行样式**：单击**每行样式**右侧的![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528933.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528934.png)图标，添加或删除一个每行内容样式。 单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528936.png)或![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528937.png)图标配置多个每行内容样式的排列方式。单击![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1161599661/p528938.png)图标，即可复制当前选中的每行样式配置并新增一个同样配置的每行样式。
...
-   **交互**：矢量散点层的交互配置项。
    ...
    **点击态**
    ...
    -   **图形**：设置**点击态**时矢量散点层图形样式，包括**矢量形状**、**散点半径**、**图标旋转**、**散点颜色**、**散点描边**。
        
    -   **标签**：设置**点击态**时矢量散点层标签，包括**显示标签**、**相对位置**、**相对偏移**、**内容样式**、**背景样式**。
    ...
    **悬浮态**
    ...
    -   **图形**：设置**悬浮态**时矢量散点层图形样式，包括**矢量形状**、**散点半径**、**图标旋转**、**散点颜色**、**散点描边**。
        
    -   **标签**：设置**悬浮态**时矢量散点层标签，包括**显示标签**、**相对位置**、**相对偏移**、**内容样式**、**背景样式**。
...
-   **条件**：条件样式说明。
    ...
    **散点描边**
    ...
    单击**相对偏移**左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制该条件下的矢量散点的描边样式的显隐，包括描边线的颜色和粗细值。
    ...
    **说明**
    ...
    -   **动作**
        ...
        **请求数据**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如矢量散点层配置了API数据源为`https://api.test`，传到**请求数据**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
