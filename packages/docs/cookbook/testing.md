...
- [store 测试 %{#testing-stores}%](#store-测试-testing-stores)
  - [对 store 进行单元测试 %{#unit-testing-a-store}%](#对-store-进行单元测试-unit-testing-a-store)
  - [对组件单元测试 %{#unit-testing-components}%](#对组件单元测试-unit-testing-components)
    - [初始 State %{#initial-state}%](#初始-state-initial-state)
    - [自定义 action 的行为 %{#customizing-behavior-of-actions}%](#自定义-action-的行为-customizing-behavior-of-actions)
    - [选择性 action 存根 %{#selective-action-stubbing}%](#选择性-action-存根-selective-action-stubbing)
    - [Mocking the returned value of an action](#mocking-the-returned-value-of-an-action)
    - [指定 createSpy 函数 %{#specifying-the-createspy-function}%](#指定-createspy-函数-specifying-the-createspy-function)
    - [Mocking getters %{#mocking-getters}%](#mocking-getters-mocking-getters)
    - [Pinia 插件 %{#pinia-plugins}%](#pinia-插件-pinia-plugins)
  - [端到端测试 %{#e2e-tests}%](#端到端测试-e2e-tests)
  - [对组件单元测试(Vue 2) %{#unit-test-components-vue-2}%](#对组件单元测试vue-2-unit-test-components-vue-2)
...
当使用 Jest，或 vitest 且设置 `globals: true` 时，`createTestingPinia` 会自动使用现有测试框架 (`jest.fn` 或 `vitest.fn`) 的 spy 函数存根 (stub) action。如果你使用的是不同的框架，你需要提供一个 createSpy 选项：
