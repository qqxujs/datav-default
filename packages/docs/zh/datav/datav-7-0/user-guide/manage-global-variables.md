...
2.  单击**新建变量**，根据如下字段，配置变量说明。
    ...
    **来源**
    ...
    全局变量的来源，包括**静态值**、**数据源请求**和**从 url 查询条件获取**。
    
    **说明**
    
    不同来源类型使用方式，请参见[使用全局变量](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/use-global-variables)。
    ...
    **过滤器**
    ...
    可选，通过添加数据过滤器进一步筛选所需数据。详情请参见[使用数据过滤器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/use-filters-in-canvas-on-pcs)。
    
    /
    
    **数据响应结果**
    
    在全局变量的参数设置完成后，系统的响应结果，不同的变量类型响应结果不同。
