...
格网聚合层是基础平面地图 4.0的子组件，支持独立的样式和数据配置，包括格网的尺寸大小、颜色和标注等，能够以聚合格网点的形式表现地理位置上的点的分布密度。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9959088371/p877605.png)

## 样式面板![{70B2ADC8-1E98-4621-82F6-96B5CD0297D7}](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9959088371/p903390.png)
...
-   **快速样式**：格网聚合层子组件具备快速样式功能，您可以根据实际应用场景选择不同的样式。
...
-   **通用**：设置格网聚合层的通用配置项。
    ...
    **可见级别**
    ...
    设置格网聚合层的可见级别值，在设定的级别范围内格网聚合层可见，级别之外不可见。
    
    **不透明度**
    
    设置格网聚合层的不透明度值，取值范围为0~100，单位%，数值越大图层越不透明。
...
-   **图形**：设置格网聚合层的图形配置项。
    ...
    **聚合半径**
    ...
    根据地图实际距离标注当前聚合图形的半径，范围为0~1000，单位km。
    
    **格网尺寸**
    
    设置格网的尺寸大小，请参见[尺寸映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/dimension-mapper-instructions)，配置不同格网的映射尺寸样式。
    
    **格网颜色**
    
    设置格网的颜色样式，请参见[颜色映射器使用说明](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/instructions-of-color-mapper)，配置不同格网的映射颜色样式。
    
    **格网描边**
    
    设置以当前格网半径为基准的格网描边线的颜色和描边线的粗细值，取值范围为0~50。
    
    **格网阴影**
    
    设置格网阴影样式的颜色、阴影的模糊度值以及阴影基于格网中心点水平方向/垂直方向的偏移。
    
    **格网标注**
    
    单击左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制格网标注样式的显隐。
    
    **文本样式**：设置标注文字的字体、文字粗细、字号和颜色。
    

## 数据源面板![{1342943E-D0F0-4213-979A-0535227B4787}](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9959088371/p901215.png)
...
![{62F5B4A4-51B9-4FBB-88F3-8ADF9D60D409}](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9959088371/p907135.png)
...
3.  查看蓝图配置参数。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/9959088371/p877621.png)
    ...
    **说明**
    ...
    -   **动作**
        ...
        **导入数据接口**
        ...
        按组件绘制格式处理数据后，导入组件，重新绘制。不需要重新请求服务端数据，查看数据示例。
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如格网聚合层配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
