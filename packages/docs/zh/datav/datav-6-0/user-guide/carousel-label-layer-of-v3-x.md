  配置轮播标签层的样式数据与交互-DataV数据可视化-阿里云
...
-   **通用**：设置轮播标签层的通用配置项。
    ...
    **可见级别**
    ...
    设置轮播标签层的可见级别值，在设定的级别范围内可见，级别之外不可见。
    
    **不透明度**
    
    轮播标签层的不透明度，范围为0~100，单位%。数值越大轮播标签层越不透明。
...
-   **图形**：设置轮播标签层的图形配置项。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6898808761/p577284.png)
    ...
    **图标形状**
    ...
    轮播标签层标签内图标的形状样式，您可以单击样式右侧的下拉选项框，选择系统内置多种图标样式；也可在下拉框面板的输入框内自定义输入图片的URL地址，将远程服务器上的图片作为标签图标；或者单击输入框右侧的![上传按键](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3366732361/p310531.jpg)图标，上传本地图片作为标签图标。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6898808761/p577285.png)
    
    **图标尺寸**
    
    设置图标在宽度和高度上的尺寸大小，取值范围为0~200，单位px。
    
    **图标偏移**
    
    设置图标在标签内的水平和垂直方向的偏移值，取值范围为-200~200，单位px。
...
-   **动效**：设置轮播标签层中的轮播效果参数。 ![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6898808761/p577287.png)
    ...
    **轮播顺序**
    ...
    设置轮播标签层的轮播顺序样式，可选，包括**随机轮播**和**顺序轮播**两种。
    
    **轮播数量**
    
    设置轮播标签层的单次轮播标签的数量值，取值范围为0~50，单位个。
    
    **轮播时长**
    
    设置轮播标签层中的单次轮播时长样式，包括轮播时的**进入时长**，取值范围为0~5，单位秒；**停留时长**，取值范围为1~500，单位秒。
    
    **轮播缓存**
    
    设置轮播标签层中轮播时可缓存的数量值，取值范围为0~100。单击**轮播缓存**左侧![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制轮播缓存功能的开启或关闭。
...
-   **标签**：设置轮播标签层内标签的样式参数。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6898808761/p577289.png)
    ...
    **相对偏移**
    ...
    设置轮播标签层的标签在水平和垂直方向的相对偏移值，单位px，取值范围为-100~100。
    ...
    **背景样式**
    ...
    设置轮播标签层内标签的背景样式参数，单击**背景样式**左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制**背景样式**的显隐。
    ...
    -   **背景阴影**：设置标签背景的阴影样式，包括阴影的颜色、阴影模糊度，阴影相对于标签在X和Y轴方向上的偏移值。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**轮播标签层**组件，在画布中可以看到如下图所示的轮播标签层的蓝图编辑器配置参数。![轮播标签层蓝图参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/4286732361/p312246.jpg)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如轮播标签层配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
