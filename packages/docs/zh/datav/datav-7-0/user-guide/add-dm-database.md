  添加达梦数据库为数据源-DataV数据可视化-阿里云

在DataV-Board（数据看板）中使用达梦数据库之前，需要先将达梦数据库实例添加为DataV-Board的数据源。本文为您介绍如何添加达梦数据库。
