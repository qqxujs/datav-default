  双11消费屏柱图组件配置与数据交互详解-DataV数据可视化-阿里云
...
-   **全局样式**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8090696761/p571199.png)
    ...
    **显示数量**
    ...
    组件页面内显示的柱子数量，由此配置项和数据项共同决定。例如设置**数量**为5，当数据面板中的数据项≤5时，组件内显示的柱子数量＝数据项的数量；当数据面板中的数据项≥5时，组件内显示的柱子数量＝配置项的**数量**。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**双11消费屏柱图**组件，在画布中可以看到如下图所示的双11消费屏柱图的蓝图编辑器配置参数。![蓝图编辑器](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2960306361/p274406.jpg)
    ...
    -   **动作**
        ...
        **请求接口描述**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如双11消费屏柱图配置了API数据源为`https://api.test`，传到**请求接口描述**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
