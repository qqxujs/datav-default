...
|    **字段**    |    **说明**    |
|    ----    |    ----    |
|    区域编号    |    区域的adcode编号，可通过[DataV.GeoAtlas](http://datav.aliyun.com/static/tools/atlas)获取。**说明** 如果需要下钻到区域热力层，则数据中必须包含下钻位置的adcode，如果您没有配置下钻区域对应的adcode则看不到区域热力层。    |
|    名称    |    （可选）地区的名称。    |
|    颜色映射字段    |    （可选）地区的颜色映射字段。    |
|    纹理形状映射字段    |    （可选）地区的纹理形状映射字段。    |
|    纹理颜色映射字段    |    （可选）地区的纹理颜色映射字段。    |
