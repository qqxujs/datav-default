  配置WMS栅格瓦片图层的样式数据与交互-DataV数据可视化-阿里云

WMS栅格瓦片是三维城市构建器的全局图层，支持独立的样式和数据配置，包括WMS栅格瓦片的通用配置、图形配置和交互配置样式。本文介绍WMS栅格瓦片配置项的含义。

在场景编辑器左侧图层列表中，单击WMS栅格瓦片图层，进入配置面板配置WMS栅格瓦片图层的样式和数据。

**说明** 如果图层列表中没有WMS栅格瓦片，说明您还未添加WMS栅格瓦片图层，请参见[添加子组件](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/layer-panel-management-of-professional-edition#section-1yb-xd4-pfe)进行添加。
...
![WMS栅格瓦片数据面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2008448461/p423262.png)
...
如果您在创建三维城市构建器项目时，使用了空间数据项目模式，并且项目内包含了WMS栅格瓦片数据内容。在场景编辑器内配置WMS栅格瓦片的数据面板时，您还可以使用空间构建数据源获取空间地图上WMS栅格瓦片数据， 具体步骤如下：
...
1.  在当前场景编辑器页面，单击左侧图层列表中WMS栅格瓦片右侧的![导出到蓝图编辑器](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89089.jpg)图标。
2.  单击页面左上角的![蓝图图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89087.jpg)图标。
3.  在蓝图编辑器配置页面，单击导入节点栏内的WMS栅格瓦片图层，在画布中可以看到如下图所示的WMS栅格瓦片的蓝图编辑器配置参数。![蓝图配置参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2008448461/p423270.png)
    -   事件
    ...
    -   动作
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如WMS栅格瓦片配置了API数据源为`http://api.test`，传到请求数据接口动作的数据为`{ id: '1'}`，则最终请求接口为`http://api.test?id=1`。
