  配置S3M管网图层的样式数据与交互-DataV数据可视化-阿里云

S3M管网是三维城市构建器的图层，支持独立的样式和数据配置，包括S3M管网的通用配置和交互配置样式。本文介绍S3M管网配置项的含义。

在场景编辑器左侧图层列表中，单击S3M管网图层，进入配置面板配置S3M管网图层的样式和数据。

**说明** 如果图层列表中没有S3M管网，说明您还未添加S3M管网图层，请参见[添加子组件](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/layer-panel-management-of-professional-edition#section-1yb-xd4-pfe)进行添加。

## 配置

![配置面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5385358461/p424038.png)
...
-   S3M管网配置面板中的参数说明。
    -   通用
        ...
        设置S3M管网的可见性像素权重，取值范围为1~100。
        
        lod更新速率
        
        设置S3M管网的lod更新速率，取值范围为1~100。
    ...
    -   交互
        ...
        打开鼠标交互的开关，可以看到是否开启iServer的交互开关、高亮颜色的配置选项以及透明度的配置项；关闭开关，则看不到鼠标交互下的配置选项。其中iServer交互当且仅当在数据源中存在interactive\_url字段时才会生效。
        

## 数据

![数据面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5385358461/p424039.png)
...
S3M管网的三维服务URL，如//%/realspace/datas/%/config。

interabtive\_url

S3M管网的数据服务URL，如//%/realspace/services/%/rest/data。
...
1.  在当前场景编辑器页面，单击左侧图层列表中S3M管网右侧的![导出到蓝图编辑器](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89089.jpg)图标。
2.  单击页面左上角的![蓝图图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2434449951/p89087.jpg)图标。
3.  在蓝图编辑器配置页面，单击导入节点栏内的S3M管网图层，在画布中可以看到如下图所示的S3M管网的蓝图编辑器配置参数。![蓝图配置参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5385358461/p424040.png)
    -   事件
        ...
        当鼠标左键单击S3M管网时抛出的事件，抛出数据为object类型。
    ...
    -   动作
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如S3M管网配置了API数据源为`http://api.test`，传到请求数据接口动作的数据为`{ id: '1'}`，则最终请求接口为`http://api.test?id=1`。
