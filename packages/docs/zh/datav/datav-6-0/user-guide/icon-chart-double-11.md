  使用与配置双11图标占比图组件-DataV数据可视化-阿里云

本文介绍双11图标占比图各配置项的含义。

## **图表样式**

双11图标占比图是其他基础图表的一种，支持自定义类别图标和内容，能够使用图标形象地展示两种类别的占比情况，适用于双11可视化应用中的类别占比展示，例如展示购买某件商品的男女比例。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1662939761/p607086.png)

## 配置面板![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1662939761/p607087.png)
...
-   **图标**
    ...
    -   **图标1**：类别A的显示图标。单击图片框删除当前图片，再次单击上传本地图片，也可以在输入框中输入图片的URL地址，来使用远程服务器上的图片。
        
    -   **图标2**：类别B的显示图标。单击图片框删除当前图片，再次单击上传本地图片，也可以在输入框中输入图片的URL地址，来使用远程服务器上的图片。
        
    -   **排列**：各类别标签的排列样式，可选：
        
        -   **竖向**：标签排列呈竖向时隐藏标签栏。
            
        -   **横向**：标签排列呈横向时显示标签栏，可修改标签值，自定义类别。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1662939761/p607088.png)
...
-   **样式**
    ...
    **图标宽度**
    ...
    各类别图标的宽度，单位为px，取值范围为0~1000。
    
    **图标高度**
    
    各类别图标的高度，单位为px，取值范围为0~1000。
    
    **文本字号**
    
    类别标签文本的字体大小，取值范围为0~100。
    
    **字体**
    
    类别标签文本的字体系列，默认为**微软雅黑**。
    
    **字体颜色**
    
    类别标签文本的字体颜色，请参见[颜色选择器说明](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/configure-item-description#section-kdw-vj4-t2b)进行修改。
    
    **图标偏移值**
    
    各图标相对于组件中心的横向偏移值，单位为px，取值范围为0~1000。
    
-   **图标文本间距**：当排列方式为纵向时，表示文本与组件下边界的距离；当排列方式为横向时，表示图标与标签文本之间的横向距离。单位为px，取值范围为0~1000。
    
-   **文本间距**：当排列方式为纵向时，表示标签文本与组件左右两侧边界的距离；当排列方式为横向时，表示标签的类别文本与数值文本之间的纵向距离。单位为px，取值范围为0~20。
...
![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1662939761/p607089.png)
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**双11图标占比图**组件，在画布中可以看到如下图所示的双11图标占比图的蓝图编辑器配置参数。![占比图蓝图编辑器参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2666271161/p69335.png)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如双11图标占比图配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
