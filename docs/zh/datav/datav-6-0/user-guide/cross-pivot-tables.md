...
|    交互事件    |    说明    |
|    ----    |    ----    |
|    **当数据单元格选中时**    |    选中**启用**，开启组件交互功能。**交叉透视表**组件拥有交互配置，当数据单元格选中时抛出回调值。具体配置请参见[如何配置数字翻牌器组件的回调ID](/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards)。    |
