...
`config`配置中的`type`字段用来配置组件的类型和样式。不同的`type`类型会展示不同的编辑器UI，合理使用`type`类型和配置项归类，可以极大提高组件的质量与用户体验，详情请参见[控件配置](/zh/datav/datav-6-0/user-guide/text#concept-2427732 "text表示组件的配置项类型为⽂本输⼊框，⽀持定制前后缀。")。 
...
|    source    |    接口名    |    否    |    可自定义。    |
|    handler    |    处理接口返回的数据的方法名    |    是    |    该方法名必须在[index.js](/zh/datav/datav-6-0/user-guide/specifications-of-an-index-js-file-1#concept-bmf-nxl-q2b "index.js文件是组件的主入口文件。本文档提供了一个示例供您参考，并介绍了index.js文件中的常用函数。")源码中实现。    |
...
* `emit`触发  
`emit`方法作为一个基础类方法，会将事件名及需要的数据作为参数抛出，其他组件即可通过变量名获取到该组件的参数值，需要在[index.js](/zh/datav/datav-6-0/user-guide/specifications-of-an-index-js-file-1#concept-bmf-nxl-q2b "index.js文件是组件的主入口文件。本文档提供了一个示例供您参考，并介绍了index.js文件中的常用函数。")源码中实现（详情请参见注释）。  
```  
render: function () {  
