...
|    **参数**    |    **说明**    |
|    ----    |    ----    |
|    **图表边距**    |    单击![开关图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1986941261/p274464.jpg)图标，控制图表边距的显隐。打开开关后，可配置区域与组件上下左右四个边界之间的距离，单位默认px。    |
|    **区域形式**    |    区域图的形式，可选**基础**和**堆叠**。    |
...
|    **参数**    |    **说明**    |
|    ----    |    ----    |
|    **显示格式**    |    期望显示的数据格式，仅对时间型数据有效。时间请参照YYYY/MM/DD HH:mm:ss，整数参照d，浮点参照.1f。    |
...
|    **参数**    |    **说明**    |
|    ----    |    ----    |
|    **触发方式**    |    区域被选中触发的方式，可选，包括**悬浮**和**点击**。    |
|    **选中方式**    |    选中区域的方式，可选，包括**单选**和**多选**。当选择**单选**时，单击事件只抛出一条数据，数据格式为对象，只能高亮一个区域；当选择**多选**时，单击事件可抛出多条数据，数据格式为数组，可高亮多个区域。    |
|    **折线类型**    |    设置折线的线型、折线的粗细和折线的颜色。单击![开关图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/1986941261/p274464.jpg)图标可开启或关闭折线线型的样式 。    |
|    **选中颜色**    |    设置选中区域的颜色。    |
|    **标记尺寸**    |    设置标记尺寸。    |
...
|    **背景趋势**    |    缩略轴上显示区域图的背景趋势样式，包括背景趋势线的类型、线粗细和线的颜色。通常和图表中折线数据趋势保持一致，仅在区域图只有一个数据系列的情况下可显示。    |
...
|    **字段**    |    **说明**    |
|    ----    |    ----    |
|    colorField    |    （可选）颜色映射字段，用于设置区域的各种颜色的映射配置样式。    |
|    x    |    用于配置区域图中每个区域的类目，即x轴的值。    |
|    y    |    用于配置区域图中每个区域的值，即y轴的值。    |
...
|    x    |    用于配置区域图中每个区域的类目，即x轴的值。    |
|    y    |    用于配置区域图中每个区域的值，即y轴的值。    |
|    系列    |    颜色映射系列值，用于设置不同系列数据下的区域颜色的映射配置样式。    |
...
|    **事件**    |    **说明**    |
|    ----    |    ----    |
|    **当折线图接口请求完成时**    |    数据接口请求返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**区域。    |
|    **当折线图接口请求失败时**    |    数据接口请求失败时（请求失败的情况可能是：网络问题或接口报错等）返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**区域。    |
|    **当数据项选中时（点）**    |    当单击区域图上的点时抛出的事件，同时抛出该点对应的数据项。    |
|    **当数据项选中时（线）**    |    当单击区域图上的线时抛出的事件，同时抛出该线对应的数据项。    |
|    **当数据项选中时（面）**    |    当单击区域图上的面时抛出的事件，同时抛出该面对应的数据项。    |
|    **当图例被点击时**    |    当单击区域图的图例时抛出的事件，同时抛出该折线对应的数据项。    |
...
|    **导入折线图接口**    |    按组件绘制格式处理数据后，导入组件，重新绘制。不需要重新请求服务端数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**区域。    |
