  为DataV添加RDS for SQLServer数据源-DataV数据可视化-阿里云

本文为您介绍在DataV-Board中通过内网和外网添加RDS for SQLServer数据源的方法。
    ...
    -   专有网络
        ...
        2.  填写数据库信息。
            ...
            **VPC ID**
            ...
            VPC实例的ID，此VPC为您的RDS for SQLServer实例所在的VPC。可进入[RDS管理控制台](https://rdsnext.console.aliyun.com/)，在**实例列表**的**网络类型**中获取。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/6289667571/p1006441.png)
            ...
            **说明**
            ...
            通常您可以直接在数据库产品的控制台查看数据库实例的VPC实例ID。但是由于VPC网络实现机制的原因，有时控制台显示的实例ID并不是DataV数据源配置 VPC网络需要的实例ID。因此您需要通过数据库产品的OPEN API获取DataV需要的VPC实例ID，通常为数据库实例详情中的VpcCloudInstanceId或VpcInstanceId，具体数据库产品获取VPC实例ID的接口，请参见[查询实例详情](https://help.aliyun.com/zh/rds/api-query-instance-details)。
            ...
            **重要**
            ...
            此处的URL地址不是官网页面的URL，也不是本机的IP，是DataV服务器能够通过公网或阿里云部分Region内网访问您数据库的URL地址。
