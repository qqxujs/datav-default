...
|    **AppKey**    |    API网关接口的调用AppKey。请参见[创建应用](/zh/api-gateway/traditional-api-gateway/getting-started/create-an-api-with-a-backend-service-of-the-http)，获取相关接口的AppKey。    |
|    **AppSecret**    |    API 网关接口的调用AppSecret。请参见[创建应用](/zh/api-gateway/traditional-api-gateway/getting-started/create-an-api-with-a-backend-service-of-the-http)，获取相关接口的AppSecret 。    |
...
阿里云API网关是面向用户的产品，您可以通过以下两种方式获取API网关接口。  
  ...
  * 将您的API托管在API网关，通过DataV调用API网关来调用您的接口，接入方式请参见[阿里云API网关官方帮助文档](/zh/api-gateway/)。
