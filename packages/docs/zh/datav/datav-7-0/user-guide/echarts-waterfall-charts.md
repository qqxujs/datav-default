...
3.  查看蓝图配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7494962961/p709457.png)
    ...
    -   **事件**
        ...
        **当点击数据项时**
        ...
        当单击瀑布图的数据时抛出的事件，同时抛出对应的数据项。
        
        **当图例点击时**
        
        当单击瀑布图的图例时抛出的事件，同时抛出对应的数据项。
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如瀑布图配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
