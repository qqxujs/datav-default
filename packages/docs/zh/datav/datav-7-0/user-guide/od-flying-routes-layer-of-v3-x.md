...
|    **飞线样式**    |    设置飞线线条的样式。**飞线线型**：单击下拉选项框，选择一种飞线的线型样式。**飞线颜色**：请参见[颜色映射器使用说明](/zh/datav/datav-7-0/user-guide/instructions-of-color-mapper)，配置不同飞线的映射颜色样式。**飞线长度**：拖动滑块设置飞线的长度值，取值范围为10\~100。**飞线粗细**：拖动滑块设置飞线的粗细值，取值范围为0\~100，单位px。**飞线弧度**：拖动滑块设置飞线的弧度矢跨比值，取值范围为0\~1。**边缘模糊**：拖动滑块设置虚化宽度比例值，取值范围为0\~1。    |
