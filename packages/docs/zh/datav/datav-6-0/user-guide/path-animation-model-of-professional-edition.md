...
**说明** 如果图层列表中没有路径动画模型，说明您还未添加路径动画模型图层，请参见[添加子组件](/zh/datav/datav-6-0/user-guide/layer-panel-management-of-professional-edition#section-1yb-xd4-pfe)进行添加。 
...
|    参数    |    说明    |
|    ----    |    ----    |
|    模型动画    |    设置路径动画模型加载的模型的动画样式。 自动播放：打开开关，开启动画播放；关闭开关，关闭动画播放。 循环模式：设置路径动画模型播放的循环模式，可选重复播放、单次播放和反向播放。 动画速率：动滑块设置路径动画模型加载的模型动画速率，取值范围为-1.5\~1.5。    |
|    扫光动画    |    打开开关，设置扫光动画样式；关闭开关，无法设置扫光动画样式。 锋面类型：设置扫光动画的锋面类型，包括平面类型和曲面类型。 扫描轴向：当锋面类型选择平面时，扫描轴向包括X、Y和Z轴。 正负方向：设置扫光动画的正负方向，包括正方向和负方向。 颜色：设置扫光动画的颜色，请参见[颜色选择器说明](/zh/datav/datav-6-0/user-guide/configure-item-description#section-kdw-vj4-t2b)修改颜色。 扫光长度：拖动滑块设置扫光动画的长度比例，取值范围为0.01\~1。 扫光速度：拖动滑块设置扫光动画的速度比例，取值范围为0\~0.05。    |
|    路径动画    |    打开开关，启用路径动画；关闭开关，停止路径动画。 自动播放：打开开关，路径动画可以自动播放；关闭开关， 路径动画暂停播放。 循环模式：设置路径动画模型播放的循环模式，可选重复播放和单次播放。 动画时间：拖动滑块设置动画播放时间，单位为ms。 显示路径：打开开关，显示路径；关闭开关，不显示路径。 路径高度：拖动滑块调整运动路径高程，取值范围为0\~5000。 曲线张力：拖动滑块调整路径曲线弧度，取值范围为0\~1，数值越大，弧度越大。    |
...
|    点击区域    |    设置鼠标单击路径动画模型加载模型的区域选项，包括局部和整体两个选择。    |
...
|    动作    |    说明    |
|    ----    |    ----    |
|    请求运动路径    |    重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如路径动画模型配置了API数据源为http://api.test，传到请求运动路径动作的数据为{ id: '1'}，则最终请求接口为htp://api.test?id=1。    |
