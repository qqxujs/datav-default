...
本文介绍音频组件全量选择时各配置项的含义。

## **图表样式**

音频是媒体组件的一种，支持自定义音频的URL地址、音频播放设置，音量设置等，支持mp4格式的音频，能够在可视化应用中添加音频播放器来播放您的音频。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3279612961/p706599.png)

## 样式面板![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3279612961/p706600.png)
...
* **音频链接**：建议使用mp4格式的音频地址（如：//player.alicdn.com/video/editor.mp4），也可以在**数据源**面板中配置，优先使用**数据源**面板中的配置。
* **播放设置**：设置音频的播放样式。
...
## 数据源面板![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3279612961/p706602.png)

`source`：（可选）配置音频资源的地址，与样式面板中的**音频链接**配置项功能相同。若两者都进行了配置，则优先使用**数据源**中的配置。
...
3. 查看蓝图配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3279612961/p706605.png)

  * **事件**

|    **事件**    |    **说明**    |
|    ----    |    ----    |
|    **当音频接口请求完成时**    |    数据接口请求返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据源**页签的**数据响应结果**区域。    |
  * **动作**

|    **动作**    |    **说明**    |
|    ----    |    ----    |
|    **请求音频接口**    |    重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如音频配置了API数据源为https://api.test，传到**请求音频接口**动作的数据为{ id: '1'}，则最终请求接口为https://api.test?id=1。    |
|    **导入音频接口**    |    按组件绘制格式处理数据后，导入组件，重新绘制。不需要重新请求服务端数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据源**页签的**数据响应结果**区域。    |
|    **开始播放**    |    开始播放音频，无需参数。    |
|    **暂停播放**    |    暂停播放音频，无需参数。    |
|    **结束播放**    |    结束播放音频，无需参数。    |
