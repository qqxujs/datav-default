  为3D平面地图添加并配置动态轨迹层（v3.x版本）-DataV数据可视化-阿里云
...
-   **图形**：设置动态轨迹层的图形样式参数项。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5288768761/p581987.png)
    ...
    **轨迹样式**
    ...
    -   **轨迹线型**：单击下拉框，选择一种轨迹线类型样式。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5288768761/p581990.png)
        
    -   **轨迹颜色**：请参见[颜色映射器使用说明](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/color-mapper-instructions#concept-2067387)，配置不同轨迹线的映射颜色样式。
    ...
    **线头样式**
    ...
    设置轨迹线线头的样式。单击**线头样式**左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制**线头样式**的显隐。
    ...
    **符号样式**
    ...
    -   **符号贴图**：设置符号贴图的类型。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5288768761/p581991.png)
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**动态轨迹层**组件，在画布中可以看到如下图所示的动态轨迹层的蓝图编辑器配置参数。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5288768761/p581997.png)
    ...
    -   **事件**
        ...
        **当数据接口请求完成时**
        ...
        数据接口请求返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**轨迹线。
        
        **当数据接口请求失败时**
        
        数据接口请求失败时（请求失败的情况可能是：网络问题或接口报错等）返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**轨迹线。
    ...
    -   **动作**
        ...
        **导入数据接口**
        ...
        按组件绘制格式处理数据后，导入组件，重新绘制。不需要重新请求服务端数据。具体数据示例请参见画布编辑器中组件右侧配置面板**数据**页签的**数据响应结果**轨迹线。
