...
-   **通用**：设置图标散点层的通用配置项。
    ...
    **不透明度**
    ...
    图标散点层的不透明度值，取值范围为0~100，单位%，数值越大图层越不透明。
...
-   **图形**：设置图标散点层的图形配置项。
    ...
    **相对位置**
    ...
    设置散点图标对于散点的相对位置样式，包括**左侧**、**右侧**、**中心**、**顶部**和**底部**。
...
-   **标签**：设置图标散点层内标签的样式参数。
    ...
    **显示方式**
    ...
    设置图标散点层的标签显示方式，包括**全显**、**点击**和**悬浮**三种。
    
    **相对位置**
    
    设置图标散点层的标签的相对位置样式，包括**左侧**、**中心**、**右侧**、**顶部**和**底部**。
...
-   **交互**：图标散点层的交互配置项。
    ...
    **点击态**
    ...
    -   **图形**：设置**点击态**时图标散点层图形样式，包括**图标形状**、**图标尺寸**、**图标旋转**、**相对位置**、**相对偏移**。
        
    -   **标签**：设置**点击态**时图标散点层标签，包括**显示标签**、**相对位置**、**相对偏移**、**内容样式**、**背景样式**。
    ...
    **悬浮态**
    ...
    -   **图形**：设置**悬浮态**时图标散点层图形样式，包括**图标形状**、**图标尺寸**、**图标旋转**、**相对位置**、**相对偏移**。
        
    -   **标签**：设置**悬浮态**时图标散点层标签，包括**显示标签**、**相对位置**、**相对偏移**、**内容样式**、**背景样式**。
...
-   **条件**：条件样式说明。
    ...
    **图标尺寸**
    ...
    拖动滑块设置图标的尺寸值，范围为0~400，单位为px。可通过![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，确认在符合当前条件时是否应用当前样式。
    
    **相对位置**
    
    设置图标与散点的相对位置样式，可通过![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，确认在符合当前条件时是否应用当前样式。
    
    **相对偏移**
    
    设置图标在水平和垂直方向的相对偏移值，取值范围为-400~400，单位为px，可通过![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，确认在符合当前条件时是否应用当前样式。
    ...
    **说明**
    ...
    -   **动作**
        ...
        **请求数据**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如图标散点层配置了API数据源为`https://api.test`，传到**请求数据**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
