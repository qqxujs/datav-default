...
-   **基础配置**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7586541861/p632459.png)
    ...
    **时间格式**
    ...
    单击下拉框，选择时间范围选择器的格式或自定义输入时间格式，格式请参照YYYY/MM/DD HH:mm:ss（年/月/日时：分：秒）。
...
-   **下拉框**![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7586541861/p632461.png)
    ...
    **禁选样式**
    ...
    设置时间范围选择器下拉框中禁选样式，包括禁选日历和时间样式的字体大小值、背景颜色和字体的颜色。
    
    **日历样式**
    
    设置时间范围选择器下拉框中可选择的日历样式。
    
    -   **子项填充**：设置日历每一个可选子项的填充色，包括默认时候的颜色、鼠标悬浮到子项时的样式和选中子项时的颜色。
        
    -   **子项文本**：设置日历每一个可选子项文本的字体样式、字的粗细值、字号大小值、文本的默认颜色、文本在鼠标悬浮时的颜色和选中文本时的颜色。
    ...
    **时间样式**
    ...
    -   **底部样式**：日历顶部的样式，包括顶部背景颜色、顶部文字的样式、顶部按键各种状态下的颜色和顶部分割线的线宽和颜色。
...
设置可选择时间范围区间（非必选项），如果数据面板中没有配置该字段数据，则默认使用\['1000-01-01 00:00:00', '5999-12-31 23:59:59'\]。

`disabledRange`

设置禁用时间范围区间（非必选项），如果数据面板中没有配置该字段数据，则默认使用\['1000-01-01 00:00:00', '5999-12-31 23:59:59'\]。

`disabled`

设置单独禁用某个时间节点（非必选项），若内容不完整, 则使用'5999-12-31 23:59:59\]。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**时间范围选择器**组件，在画布中可以看到如下图所示的时间范围选择器的蓝图编辑器配置参数。![蓝图参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3476216461/p410415.jpg)
    ...
    -   **事件**
        ...
        **当前时间区间变化时**
        ...
        当前时间区间值发生变化时抛出的事件，同时抛出该值对应的数据项。
