...
|    **参数**    |    **说明**    |
|    ----    |    ----    |
|    **不透明度**    |    设置柱状热力层的不透明度值，取值范围为0\~1。数值越大柱状热力层样式越不透明。    |
* **图形**：设置柱状热力层的图形配置。  

|    **参数**    |    **说明**    |
|    ----    |    ----    |
|    **类型**    |    柱状热力层的图形类型，包括**蜂窝**和**网格**类型。    |
|    **聚合函数**    |    选择不同的聚合函数，地图会通过聚合函数重新计算，改变地图的显示样式，聚合函数包括但不限于**计数（COUNT）**、**求和（SUM）**、**均值（AVG）**、**最大值（MAX）**、**最小值（MIN）**。    |
|    **图层半径**    |    根据地图实际距离标注当前图形的半径，包括**蜂窝半径**和**网格半径**两种，取值范围20\~500，单位km。    |
|    **图形间距**    |    图形之间的空间距离，包括**蜂窝间距**和**网格间距**两种，取值范围为0\~100，单位km。    |
|    **柱体高度**    |    热力层柱体的高度，取值范围为1\~5000，单位km。    |
...
|    **顶面样式**    |    设置柱状热力层顶面样式参数。**金属度**：拖动滑块设置柱状热力层顶面的金属度，取值范围为0\~1。**粗糙度**：拖动滑块设置柱状热力层顶面的粗糙度，取值范围为0\~1。**纹理贴图**：柱子顶面的纹理贴图样式。您可以在选择框中选择或上传需要使用的贴图。**发光效果**：柱子顶面的发光贴图样式。您可以在选择框中选择或上传需要使用的贴图，调整发光效果的颜色和强度。单击**发光贴效果**左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制**发光效果**样式的显隐。    |
|    **侧面样式**    |    设置柱状热力层侧面样式参数。**金属度**：拖动滑块设置柱状热力层顶面的金属度，取值范围为0\~1。**粗糙度**：拖动滑块设置柱状热力层顶面的粗糙度，取值范围为0\~1。**纹理贴图**：柱子顶面的纹理贴图样式。您可以在选择框中选择或上传需要使用的贴图。**发光效果**：柱子侧面的发光贴图样式。您可以在选择框中选择或上传需要使用的贴图，设置发光效果水平和垂直方向的动画速率，调整发光效果的颜色和强度，可以增强视觉吸引力。单击**发光效果**左侧的![眼睛图标](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8648413261/p89095.jpg)图标，可控制**发光效果**样式的显隐。    |
* **动效**：柱状热力层的交互动态效果配置项。
