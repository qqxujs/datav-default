...
选中**当前时间变化时**右侧的**启用**，开启组件交互功能。**时间选择器**组件拥有交互配置，可改变选择器中的时间数据内容，触发数据请求，抛出回调值，然后在其他组件中配置使用。默认抛出数据面板中配置的`time`、`startTime`和`endTime`值，具体配置请参见[组件回调ID配置](/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards#concept-m2n-fy3-52b)。
