...
-   **网格**：单击右侧的**眼睛**图标显示网格样式。
    ...
    -   **背景色**：单击下拉列表，选择网格背景颜色的填充样式，包括**颜色填充**和**渐变填充**。参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改网格的背景颜色，默认透明。
        
    -   **边框色**：单击下拉列表，选择网格边框颜色的填充样式，包括**颜色填充**和**渐变填充**。参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改网格的边框颜色，默认透明。
...
-   **提示框**：单击右侧的**眼睛**图标显示提示框。
    ...
    -   **坐标轴指示器**
        ...
        -   **标签**：单击右侧的**眼睛**图标显示坐标轴指示器的标签。
            ...
            -   **背景色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改坐标轴指示器文本标签的背景颜色。
                
            -   **边框色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改坐标轴指示器文本标签的边框颜色。
            ...
            -   **阴影样式**
                ...
                -   **颜色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改阴影的颜色。
                    
                -   **透明度**：单击**+**或**\-**号，或手动输入数值，修改阴影的透明度。范围为 \[0,1\]。
                    
            -   **十字准星样式**
                
                -   **颜色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改十字准星的颜色。
...
-   **系列**
    ...
    -   **元素样式**
        ...
        -   **普通项**
            ...
            -   **边框色**：参考[颜色选择器](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/configuration-items#section-kdw-vj4-t2b)，修改普通项元素的边框色。
...
打开开关，开启组件交互功能。当单击选中某个数据项时抛出回调值，默认抛出数据中的`name`和`value`字段。具体配置请参见[组件交互配置](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。

**当选择饼图时**

打开开关，开启组件交互功能。当选中某个饼图时抛出回调值，默认抛出数据中的`selected`和`name`字段。具体配置请参见[组件交互配置](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。
...
3.  查看蓝图配置参数。![image.png](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/5285772961/p709916.png)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如玫瑰图配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
        ...
        **饼图扇区选中状态控制**
        ...
        动作传入数据为object类型，参数包括`type`（扇形动作类型）和`seriesName`（数据名）等。参考数据示例
