  使用RTMP视频流播放器组件-DataV数据可视化-阿里云

本文介绍RTMP视频流播放器组件的图表样式和各配置项的含义。

**重要**

RTMP视频流播放器组件暂时不支持在谷歌浏览器中播放视频，RTMP是依赖flash的，谷歌浏览器不支持flash ，您可以转换成hls格式，用阿里云流播放器组件播放视频，详情请参见[阿里云流播放器](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/alibaba-cloud-stream-player)。

## **图表样式**

RTMP视频流播放器是媒体组件的一种，支持自定义视频的URL地址和类型，能够在可视化应用中添加视频播放器来播放您的视频。RTMP视频流播放器组件只支持RTMP协议的视频流。

## 配置面板

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0333081861/p633315.png)
...
-   **视频流类型**：播放的视频流格式类型，可选**rtmp/mp4**和**rtmp/flv**。
...
![默认面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8087662461/p390243.jpg)
...
（可选） 视频流地址，与**视频流地址**配置项功能相同。如果同时配置，则以`source`字段中的内容为准。

`type`

（可选） 视频流类型，与**视频流类型**配置项功能相同。如果同时配置，则以`type`字段中的内容为准。
...
在使用DataV数据集提供的BI分析功能之前，系统会对RTMP视频流播放器进行以下变更。

-   数据源**RTMP视频流播放器数据接口**由静态数据切换至DataV数据集。
    
-   更新DataV数据集配置（此操作不会使字段配置丢失）。
    

![分析面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8087662461/p390245.jpg)
...
视频流地址，与**视频流地址**配置项功能相同。如果同时配置，则以`source`字段中的内容为准。

`视频流类型`

视频流类型，与**视频流类型**配置项功能相同。如果同时配置，则以`type`字段中的内容为准。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**RTMP视频流播放器**组件，在画布中可以看到如下图所示的RTMP视频流播放器的蓝图编辑器配置参数。![RTMP视频流播放器蓝图编辑器参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8216412161/p69777.jpg)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如RTMP视频流播放器配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
