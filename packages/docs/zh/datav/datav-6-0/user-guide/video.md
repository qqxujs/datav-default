  配置视频组件的样式数据与蓝图交互-DataV数据可视化-阿里云

本文介绍视频组件的图表样式和各配置项的含义。

## **图表样式**

视频是媒体组件的一种，支持自定义视频的URL地址、视频播放属性、播放器的外观属性等，支持mp4格式的视频，能够在可视化应用中添加视频播放器来播放您的视频。![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0005081861/p633424.png)

## 配置面板

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0005081861/p633426.png)
...
-   **视频地址**：建议使用mp4格式的视频地址（如：xx.test.com/video.mp4），也可以在**数据**面板中配置，优先使用数据面板中的配置。
    
-   **空视频文案**：在没有具体视频时，用来提醒用户的文本，一般显示在组件内部上方。
    
-   **自动播放**：打开开关，在预览或发布页面视频可自动播放。
    
    **重要**
    
    -   当您使用Chrome 66以下版本时，需要按照以下方法开启自动播放功能，该配置项才有效（详情请参见[官方文档](https://developers.google.com/web/updates/2017/09/autoplay-policy-changes)）。
        
        在Chrome浏览器地址栏中输入**chrome://flags/#autoplay-policy**，将**Autoplay policy**配置项设置为**No user gesture is required**。
        
    -   当您使用Chrome 66及以上版本时，默认自动静音播放。
        
    
-   **循环播放**：打开开关，在预览或发布页面视频播放完毕后，再次播放。
    
-   **控制条**：打开开关，在预览或发布页面视频底部可显示控制条。
    
-   **静音**：打开开关，视频静音播放。
    
-   **音量（%）**：视频播放时的声音大小。拖动控制条可在可视化应用中调节音量的大小。只有在关闭**静音**配置时生效。
...
![视频的数据面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/7095531461/p12871.png)
...
在使用DataV数据集提供的BI分析功能之前，系统会对视频进行以下变更。

-   数据源**视频数据接口**由静态数据切换至DataV数据集。
    
-   更新DataV数据集配置（此操作不会使字段配置丢失）。
    

![image](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/0005081861/p633431.png)
...
`视频地址`：（可选）配置视频资源的地址，与**视频地址**配置项功能相同。若两者都进行了配置，则优先使用数据中的配置。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的**视频**组件，在画布中可以看到如下图所示的视频的蓝图编辑器配置参数。![视频蓝图编辑器参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8095531461/p69875.jpg)
    ...
    -   **动作**
        ...
        **请求数据接口**
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如视频配置了API数据源为`https://api.test`，传到**请求数据接口**动作的数据为`{ id: '1'}`，则最终请求接口为`https://api.test?id=1`。
