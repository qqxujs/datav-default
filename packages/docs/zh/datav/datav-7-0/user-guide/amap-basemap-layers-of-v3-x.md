...
打开开关，开启组件交互功能。可单击某个室内地图楼层，触发数据请求，抛出回调值，实现在切换楼层的时候，动态加载不同楼层的数据。 默认抛出`indoormapFloor`字段。具体配置请参见[组件交互配置](/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。
