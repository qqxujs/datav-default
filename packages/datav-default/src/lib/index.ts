...
import { TypeControl, type TypeConfigAll } from './.types/index'
...
export function getDefaultConfigForItem(config: TypeConfigAll) {
  ...
  if (config.type === TypeControl.boolean)
    return getDefaultConfigForBoolean(config as TypeConfigBoolean)
  // 基础输入控件 - keyBoard 热键定义器
  if (config.type === TypeControl.keyBoard)
    return getDefaultConfigForKeyBoard(config as TypeConfigKeyBoard)
  // 基础输入控件 - number 数值输入框
  if (config.type === TypeControl.number)
    return getDefaultConfigForNumber(config as TypeConfigNumber)
  // 基础输入控件 - percent 百分比值
  if (config.type === TypeControl.percent)
    return getDefaultConfigForPercent(config as TypeConfigPercent)
  // 基础输入控件 - text 文本输入框
  if (config.type === TypeControl.text)
    return getDefaultConfigForText(config as TypeConfigText)

  // 选择型控件 - buttonRadio 单选按钮
  if (config.type === TypeControl.buttonRadio)
    return getDefaultConfigForButtonRadio(config as TypeConfigButtonRadio)
  // 选择型控件 - checkbox 复选框
  if (config.type === TypeControl.checkbox)
    return getDefaultConfigForCheckbox(config as TypeConfigCheckbox)
  // 选择型控件 - iconRadio 单选图标
  if (config.type === TypeControl.iconRadio)
    return getDefaultConfigForIconRadio(config as TypeConfigIconRadio)
  // 选择型控件 - imageSelect 图片选择器
  if (config.type === TypeControl.imageSelect)
    return getDefaultConfigForImageSelect(config as TypeConfigImageSelect)
  // 选择型控件 - menu 菜单
  if (config.type === TypeControl.menu)
    return getDefaultConfigForMenu(config as TypeConfigMenu)
  // 选择型控件 - radio 单选框
  if (config.type === TypeControl.radio)
    return getDefaultConfigForRadio(config as TypeConfigRadio)
  // 选择型控件 - search 下拉搜索框
  if (config.type === TypeControl.search)
    return getDefaultConfigForSearch(config as TypeConfigSearch)
  // 选择型控件 - select 选择器
  if (config.type === TypeControl.select)
    return getDefaultConfigForSelect(config as TypeConfigSelect)
  // 选择型控件 - switch 开关
  if (config.type === TypeControl.switch)
    return getDefaultConfigForSwitch(config as TypeConfigSwitch)
  // 选择型控件 - timePicker 时间选择器
  if (config.type === TypeControl.timePicker)
    return getDefaultConfigForTimePicker(config as TypeConfigTimePicker)
  // 选择型控件 - datePicker 日期选择器
  if (config.type === TypeControl.datePicker)
    return getDefaultConfigForDatePicker(config as TypeConfigDatePicker)

  // 数值调节型控件 - line 线套件
  if (config.type === TypeControl.line)
    return getDefaultConfigForLine(config as TypeConfigLine)
  // 数值调节型控件 - slider 滑动条
  if (config.type === TypeControl.slider)
    return getDefaultConfigForSlider(config as TypeConfigSlider)
  // 数值调节型控件 - stepper 步进器
  if (config.type === TypeControl.stepper)
    return getDefaultConfigForStepper(config as TypeConfigStepper)

  // 样式配置型控件- color 颜色选择器
  if (config.type === TypeControl.color)
    return getDefaultConfigForColor(config as TypeConfigColor)
  // 样式配置型控件- fill 填充框
  if (config.type === TypeControl.fill)
    return getDefaultConfigForFill(config as TypeConfigFill)
  // 样式配置型控件- font 字体套件
  if (config.type === TypeControl.font)
    return getDefaultConfigForFont(config as TypeConfigFont)
  // 样式配置型控件- image 图片框
  if (config.type === TypeControl.image)
    return getDefaultConfigForImage(config as TypeConfigImage)
  // 样式配置型控件- multicolor 渐变颜色选择器
  if (config.type === TypeControl.multicolor)
    return getDefaultConfigForMulticolor(config as TypeConfigMulticolor)
  // 样式配置型控件- tabs 标签组
  if (config.type === TypeControl.tabs)
    return getDefaultConfigForTabs(config as TypeConfigTabs)

  // 布局容器型控件 - array 数据系列
  if (config.type === TypeControl.array)
    return getDefaultConfigForArray(config as TypeConfigArray)
  // 布局容器型控件 - group 配置项组
  if (config.type === TypeControl.group)
    return getDefaultConfigForGroup(config as TypeConfigGroup)
  // 布局容器型控件 - margin 外边距
  if (config.type === TypeControl.margin)
    return getDefaultConfigForMargin(config as TypeConfigMargin)
  // 布局容器型控件 - padding 内边距
  if (config.type === TypeControl.padding)
    return getDefaultConfigForPadding(config as TypeConfigPadding)
  // 布局容器型控件 - suite 通用套件
  if (config.type === TypeControl.suite)
    return getDefaultConfigForSuite(config as TypeConfigSuite)
  // 特殊功能型控件 - hidden 隐藏配置项
  if (config.type === TypeControl.hidden)
    return getDefaultConfigForHidden(config as TypeConfigHidden)
  // 特殊功能型控件 - show 显示条件
