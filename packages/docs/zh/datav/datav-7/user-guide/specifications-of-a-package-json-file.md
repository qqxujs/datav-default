...
`config`配置中的`type`字段用来配置组件的类型和样式。不同的`type`类型会展示不同的编辑器UI，合理使用`type`类型和配置项归类，可以极大提高组件的质量与用户体验，详情请参见[控件配置规范](https://help.aliyun.com/zh/datav/datav-7/user-guide/control-configurations-1)。
...
-   `apis`字段
    ...
    该方法名必须在[index.js规范](https://help.aliyun.com/zh/datav/datav-7/user-guide/specifications-of-an-index-js-file)源码中实现。
