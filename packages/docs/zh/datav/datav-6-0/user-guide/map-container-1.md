  3D平面中国地图的地图容器配置数据与交互-DataV数据可视化-阿里云
...
-   填充设置![填充配置面板](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/2299804561/p446187.png)
    -   填充颜色
        ...
        在数据面板中，value值最小时对应区域的颜色，请参见[颜色选择器说明](https://help.aliyun.com/zh/datav/datav-6-0/user-guide/configure-item-description#section-kdw-vj4-t2b)进行修改。
        
        最大值
        
        在数据面板中，value值最大时对应区域的颜色。
        
        无数据
        
        数据面板中未定义value字段时，对应区域的颜色。
        
        **说明** DataV会根据数据面板中value字段值大小，结合您设置的最大值、最小值和无数据颜色，来渲染不同区域的颜色。
        
        背面
        
        地图背面的颜色，以及旋转至背面显示的颜色。
...
地区对应的数值，与最大值、最小值 和无数据配置项相匹配来渲染区域的颜色。
...
3.  在蓝图编辑器配置页面，单击导入节点栏内的3D平面中国地图组件，在画布中可以看到如下图所示的3D平面中国地图的蓝图编辑器配置参数。![3D平面中国地图蓝图参数](https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/8611884261/p72118.png)
    ...
    -   动作
        ...
        重新请求服务端数据，上游数据处理节点或图层节点抛出的数据将作为参数。例如3D平面中国地图配置了API数据源为`http://api.test`，传到请求热力值数据接口动作的数据为`{ id: '1'}`，则最终请求接口为`http://api.test?id=1`。
