...
|    **事件**    |    **说明**    |
|    ----    |    ----    |
|    **当基本饼图接口请求完成时**    |    数据接口请求返回并经过过滤器处理后抛出的事件，同时抛出处理后的JSON格式的数据。具体数据可参见上文的[数据示例](#section-vzq-pvr-q2b)。    |
|    **当点击数据项时**    |    当单击基本饼图的扇形时抛出的事件，同时抛出该扇形对应的数据项。    |
  * **动作**
...
|    **导入基本饼图接口**    |    按组件绘制格式处理数据后，导入组件，重新绘制。不需要重新请求服务端数据。具体数据可参见上文的[数据示例](#section-vzq-pvr-q2b)。    |
