...
export function getDefaultConfigForFill(config: TypeConfigFill): any {
  const components: TypeConfigFillComponents[] = config.components || [
    TypeConfigFillComponents.flat,
  ]
  ...
  if (components.length == 1) {
    if (components[0] == TypeConfigFillComponents.flat) {
      ...
      components[0] == TypeConfigFillComponents.linearGradient &&
      ...
      components[0] == TypeConfigFillComponents.image &&
      isObject(defaultValue)
    ) {
      return defaults(
        {
    ...
    if (defaultValue && defaultValue.type && defaultValue.value) {
      ...
      components.indexOf(TypeConfigFillComponents.linearGradient) > -1
    ) {
      return {
        type: 'linearGradient',
        value: {
