...
**当点击数据项时**：打开开关，开启组件交互功能。当单击分组柱状图的柱子时，会触发数据请求，抛出临时变量，动态加载不同柱子的数据。默认抛出数据中的`x`、`y`和`s`值，具体配置请参见[组件交互配置](/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。 
