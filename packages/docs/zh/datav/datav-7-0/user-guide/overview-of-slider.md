...
打开开关，开启关联全局变量功能，可在改变当前输入框内的数值时，触发数据请求，抛出临时变量，在其他组件中配置使用。默认抛出数据源面板中配置的`value`值，具体配置请参见[组件交互配置](https://help.aliyun.com/zh/datav/datav-7-0/user-guide/widget-interaction-configuration)。
