...
|    **交互**    |    **说明**    |
|    ----    |    ----    |
|    **当数据项选中时**    |    选中**启用**，开启组件交互功能。**百分比柱状图**组件拥有交互配置，当单击选中某个数据项时抛出回调值，默认抛出数据中的x和y字段。具体配置请参见[如何配置数字翻牌器组件的回调ID](/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards)。    |
|    **当图例被点击时**    |    选中**启用**，开启组件交互功能。**百分比柱状图**组件拥有交互配置，当单击某个图例时抛出回调值，默认抛出数据中的value和checked字段。具体配置请参见[如何配置数字翻牌器组件的回调ID](/zh/datav/datav-6-0/user-guide/configure-callback-ids-for-ticker-boards)。    |
